final Map<String, String> am = {
  'app_name': 'Taxiye Passenger',

  // Introduction strings
  'continue': 'ቀጥል',
  'next': 'ቀጣይ',
  'select_language': 'እባክዎን ቋንቋዎን ይምረጡ',
  'skip': 'ማዘለያውን ይጫኑት',
  'warning': 'ይጠንቀቁ',

  'intro0_title': 'ታክሲ ይፈልጋሉ',
  'intro0_info': 'ፈጣንና እና ቀላል የታክሲ አገልግሎት በእጅዎ ላይ.',
  'intro1_title': 'ከከተማ ወደ ከተማ ጉዞ',
  'intro1_info': 'በተመጣጣኝ ዋጋ ከከተማ ወደ ከተማ ይጓዙ.',

  'intro2_title': 'በሰዓት ይከራዩ',
  'intro2_info': 'በሁሉም ቦታዎች ታክሲዬን ይዘዙ',
  'intro3_title': 'መልክተኛ',
  'intro3_info': 'የመልእክት ጥቅሎችን ይላኩ እና ሌሎችም አገልግሎቶች',

  // Auth Strings
  'signup': 'ይመዝገቡ',
  'signin': 'ይግቡ',
  'passcode': 'የይለፍ ምስጢር ቁጥር',
  'forgot_passcode': 'የይለፍ ምስጢር ቁጥሩን ረስተዋል ?',

  'get_started': 'ጉዞዎን ይጀምሩ',
  'let_get_started': 'እንጀምር!',
  'signup_info': 'ስልክ ቁጥርዎን በማስገባት ጉዞውን ያስጀምሩ!',
  'phone_number': 'ስልክ ቁጥር',

  'welcome_back': 'በድጋሚ እንኳን ደህና መጡ!',
  'signin_info': 'ስልክ እና የይለፍ ቁጥርዎን ያስገቡ.',

  'terms_conditions': 'ደንብ እና ሁኔታዎች',
  'terms_info': 'በመቀጠልዎ ከታክሲዬ ጋር ተስማምተዋል',
  'signup_with': '- ወይም በዚህ ይመዝገቡ -',
  'already_have_account': 'ከዚህቀደም ተመዝግበዋል?',
  'not_regeitered': 'ምዝገባው አልተካሄደም?',

  'logout_success': 'በተሳካ ሁኔታ ከመተግበሪያው ወጥተዋል',

  // verify
  'verify_title': 'ስልክዎን ያረጋግጡ',
  'verify_info': 'በስልክ ቁጥር የተላከውን ባለ አራት አኅዝ የማረጋገጫ ቁጥሮችን ያስገቡ.',
  'reset_code_in': 'በድጋሚ ይጠይቁ',
  'didnt_recieve_code': 'የማረጋገጫ ቁጥሩ አልተላከም?',

  'resend_code': 'የምስጢር ቁጥሩን በድጋሚ ላክ',
  'call_me': 'ይደውሉልኝ',
  'token_expired': 'የቆይታ ጊዜው አልፏል እባክዎ እንደገና ይግቡ.',

  // setup profile
  'setup_profile': 'መረጃዎን ያሟሉ',
  'setup_profile_info': 'እባክዎ ትክክለኛውን መረጃ መስጠትዎን ያረጋግጡ.',
  'full_name': 'ሙሉ ስም',
  'email_address': 'የኢሜል አድራሻ',

  'gender': 'ጾታ',
  'male': 'ወንድ',
  'female': 'ሴት',
  'other': 'ሌላ',
  'crop_image': 'ምስሎን ያስተካክሉ',

  'camera': 'መቅረጸ ምስል',
  'gallery': 'ማዕከለ ምስል',

  // Profile Strings
  'your_profile': 'የእርሶ መገለጫ',
  'your_info': 'ስለራስዎ መረጃ',
  'view_edit_your_details': 'ዝርዝር መረጃዎን ለመመልከት እና ለማስተካከል',
  'your_feedback': 'የእርስዎ አስተያየት ',
  'customers_ride_feedback': 'የደንበኞች አስተያየት',
  'emergency_contacts': 'የአደጋ ጊዜ ተጠሪ መረጃ',
  'country': 'ሀገር',

  'saved_places': 'የተመዘገቡ ቦታዎች',
  'privacy_settings': 'ግላዊ ማስተካከያ',
  'enter_name': 'ስም ያስገቡ',

  'profile_picture': 'የደንበኛ መግለጫ ምስል',
  'cancel': 'ሰርዝ',
  'ok': 'እሺ',

  'add_new_contact': 'አዲስ ስም እና ስልክ ይመዝግቡ',
  'add_new_place': 'አዲስ ቦታ ይመዝግቡ',
  'home': 'ቤት',
  'work': 'ስራ',
  'pinned_on_map': 'ካርታው ላይ ያመልክቱ',

  'ratings': 'ደረጃ ይስጡ',
  'rides_completed': 'ጉዞውን አጠናቀዋል',
  'rides_cancelled': 'ጉዞው ተሰርዟል',
  'total_money_spent': 'አጠቃላይ ሂሳብ ',

  'remove_emergency_contact': 'የአደጋ ጊዜ ተጠሪዎን ይሰርዙ ?',
  'remove_emergency_contact_info': 'እርግጠኛ ነዎት የአደጋ ጊዜ ተጠሪዎን ለመሰረዝ ይፈልጋሉ ?',

  'remove_emergency_contact_success': 'የአደጋ ጊዜ ተጠሪዎን በትክክል ሰርዘዋል',
  'confirm_contact': 'ግንኙነትን ያረጋግጡ',
  'as_emergency_contact': 'እንደ አደጋ ጊዜ ተጠሪ?',

  'add_emergency_contact_success': 'እንደ አደጋ ጊዜ ተጠሪ ተመዝግቧል',
  'used': 'ተጠቃሚ',
  'times': 'ጊዜ',

  'recent_locations': 'የቅርብ ጊዜ ቦታዎች',
  'contact_permission_error': 'የተጠሪ ፍቃድ ያስፈልጋል',

  'remove_address': 'አድራሻ ያስወግዱ?',
  'remove_address_info': 'እርግጠኛ ነዎት አድራሻውን ለማስወገድ ይፈልጋሉ?',

  // privacy settings strings
  'transaction_updates': 'የግብይት ሁኔታ',
  'transaction_updates_info': 'ማረጋገጫ ፣ መድረስ ፣ የክፍያ ሁኔታ እና ሌሎችም',
  'rides': 'ጉዞዎች',
  'rides_info': 'የተለዩ ቅናሾች ፣ በከተማዎ ያሉ ልዩ እድሎች እና ሌሎችም',
  'delivery': 'መልእክት',
  'delivery_info': 'የደረሱበትን የጉዞ ጥሪ ይቀበሉ',

  // setup passcode
  'setup_passcode': 'የይለፍ ምስጥር ቁጥር ያስገቡ',
  'setup_passcode_info': 'ባለ አራት አሃዝ የይለፍ ቁጥሩን ያስገቡ ለወደፊት ይህን አይርሱ .',
  'confirm_passcode': 'የይለፍ ቁጥሩን ያረጋግጡ',
  'finish': 'ይጨርሱ',

  // Drawer items
  'my_wallet': 'ያሎት ሂሳብ',
  'manage_your_wallet': 'ሂሳብዎን ያስተዳድሩ',
  'promotions': 'ሽልማቶች',
  'promotions_info': 'የተለያዩ ሽልማቶች እዚህ ያግኙ',

  'my_orders': 'የእኔ ትዕዛዞች',
  'riview_orders': 'ጉዞዎን ይገምግሙ',
  'my_drivers': 'አሽከርካሪዬ',
  'drivers_info': 'የእርሶ ምርጥ አሽከርካሪ',

  'payment_method': 'የአከፋፍል ዘዴ',
  'payment_info': 'በተለያዩ አማራጮች ይክፈሉ',
  'app_settings': 'የመተግበሪያ ማስተካከያ',
  'adjust_preference': 'በራስዎ ምርጫ ያስተካክሉ',

  'legals': 'ህግጋቶች',
  'legals_info': 'የሚነበቡ ጠቃሚ መረጃዎ',
  'speak_real_person': 'የጥሪ ማእከል ሰራተኛ ያነጋግሩ',
  'speak_person_info': 'መረጃ ለመጠየቀና ታክሲ ለማዘዝ',

  'email_support': 'የኢሜል ድጋፍ',
  'email_contact_us': 'በኢሜል ያግኙን',
  'passenger_email_support': 'የተጓዥ የኢሜል ድጋፍ',

  // home strings
  'where_to': 'ወዴት መሄድ ይፈልጋሉ ?',
  'now': 'አሁን',
  'ride': 'ጉዞ',

  'rental': 'ኪራይ',
  'outStation': ' ከከተማ ውጪ',
  'ride_cancel_success': 'ጉዞው ተሰርዟል',

  // Map Strings
  'current_location': 'አሁን ያሉበት አካባቢ',
  'confirm': ' ያረጋግጡ ',

  // pick locations string
  'drop_off_location': 'መድረሻ ቦታ',
  'enter_location': 'መድረሻ ቦታዎን ያስገቡ...',
  'choose_on_map': 'በካርታው ላይ ይምረጡ',
  'add': 'ተጨማሪ አስገባ',
  'bf_house': 'የቅርብ ጓደኛ/ቤተሰብ አድራሻ',

  'pick_up_location': 'መነሻ ቦታ ',

  // Location detail strings
  'your_location': 'ያሉበት ቦታ',
  'add_more_stops': 'ተጨማሪ መዳረሻዎች ',
  'choose_vehicles': 'የመኪና ዓይነት ይምረጡ',
  'km': 'ኪ.ሜ',
  'min': 'ደቂቃ ',

  // select car strings
  'available_vehicles': 'አሁን የሚገኙ መኪናዎች',
  'normal': 'መደበኛ',
  'shared': 'የጋራ',
  'corporate': ' ኮርፖሬት ',

  'birr': ' ብር',
  'cash': ' ጥሬ ገንዘብ',
  'offers': ' ቅናሾች',
  'notes': ' ማስታወሻ ',
  'book_now': ' አሁኑኑ ይዘዙን ',
  'schedule_ride': 'የቀጠሮ ጉዞ',

  'schedule_ride_success': 'የቀጠሮ ጉዞዎ ተመዝግቧል',
  'upcoming_ride_scheduled': 'ቀጣይ ጉዞዎ ተመዝግቧል!',
  'pick_offer': 'ቅናሽ ይምረጡ',

  // vehicle detail
  'fare_details': 'ዋጋ ዝርዝር',
  'close': 'ይዝጉ',
  'base_fare': 'መነሻ ዋጋ',

  'per_kilometer': 'በኪ.ሜ ',
  'per_minute': 'በደቂቃ',
  'congestion_charges': 'በመንገድ መጨናነቅ የቆጠረ ሂሳብ',
  'delivery_fare': 'የመላኪያ ዋጋ',

  // Looking drivers string
  'looking_for_drivers': 'አሽከርካሪዎችን እየፈለግን ነው',
  'ride_starts_soon': 'ጉዞዎ በቅርቡ ይጀምራል',
  'delivery_starts_soon': 'መልእክትዎን ማድረስ በቅርቡ ይጀምራል',
  'side_to_cancel': 'ለመሰረዝ ይህንን ይጫኑ',

  'driver_on_route': 'አሽከርካሪው በመንገድ ላይ ነው!',
  'ride_started': 'ጉዞዎ ተጀምሯል!',
  'delivery_started': 'መልእክትዎ በመንገድ ላይ ነው!',

  // Trip started strings
  'emrgency_status': 'የአደጋ ጊዜ ሁኔታ',
  'change_emergency_status': 'የአደጋ ጊዜ ሁኔታውን  ይቀይሩ',
  'emrgency_enabled_success': 'ማንቂያው ታክሲዬ ደርሷል፤  እየሰራንበት ነው!',
  'emergency_disabled_success': 'አደጋ ጊዜ ሁኔታ ተሰርዟል',

  // Trip details strings
  'arrived_destination': 'መድረሻዎ ላይ ደርሰዋል.',
  'order_delivered': 'የላኩት መልእክት ደርሷል.',
  'pay_with_cash': 'በጥሬ ገንዘብ ይክፈሉ',
  'distance': 'ርቀት',
  'duration': 'ቆይታ',

  'inc_tax': 'ታክስን ጨምሮ',
  'cash_payment': 'የጥሬ ገንዘብ ክፍያ',
  'cbe_bank': 'ኢ.ን.ባንክ',
  'kms': 'ኪ.ሜ',
  'mins': 'ደቂቃ',

  'rate_driver_success': 'አሽከርካሪያችን በተሳካ ሁኔታ ደረጃ ተሰጥቶታል',
  'address_label_hint': 'መለያዎች ለምሳሌ፤ የጓደኛ ቤት',
  'address_name': 'የአድራሻ ስም',

  'add_place_success': 'ቦታ በተሳካ ሁኔታ ተመዝግቧል',
  'delete_place_success': 'ቦታ በተሳካ ሁኔታ ተሰርዟል',
  'edit_place_success': 'ቦታ በተሳካ ሁኔታ ተሻሽሏል',

  // trip feedback strings
  'block': 'አግድ',
  'add_to_favourites': 'ወደ ምርጦች ስብስብ ይጨምሩ',
  'submit': 'አስገባ',
  'how_was_trip': 'ጉዞው እንዴት ነበር?',
  'how_was_delivery': 'የመልእክት አገልግሎቱ እንዴት ነበር?',

  'feadback_info': 'የአርስዎ አስተያየት ቀጣዩን ጉዞዎን እንድናሻሽል ይረዳናል',
  'delivery_feadback_info': 'የአርስዎ አስተያየት ቀጣዩን አገልግሎት እንድናሻሽል ይረዳናል.',
  'additional_comments': 'ተጨማሪ አስተያየት ካለዎት...',
  'add_favourite_driver_success': 'አሽከርካሪው ወደ ምርጦች ስብስብ ተጨምሯል',
  // delivery strings
  'what_do_you_need': 'ምን ይፈልጋሉ?',
  'delivery_hint': 'ስለሚልኩት መልእክት የተወሰነ ነገር ይንገሩን...',
  'upload_pictures': 'ፎቶ ያስገቡ',
  'deliver_to_me': 'መልእክቱ የሚመጣው ወደ እኔ ነው.',

  'recievers_number': 'የተቀባይ ስልክ ቁጥር ',
  'delivery_type': 'የመልእክቱ ዓይነት',
  'order_now': 'አሁን እዘዝ',

  'order_delivery_succes': 'ትዕዛዙ በተሳካ ሁኔታ ተከናውኗል',
  'cancel_order_success': 'ትዕዛዝዎ ተሰርዟል',
  'cancel_reason': 'የተሰረዘበት ምክንያት',

  'delivery_feedback_success': 'ስለ ጠቃሚ አስተያየትዎ እናመሰግናለን.',

  // Wallet strings
  'your_wallet': 'ሂሳብዎ',
  'taxiye_cash': 'ታክሲዬ ጥሬ ገንዘብ',
  'tc_apply': 'T&C ይተገበራል',
  'balance': 'ቀሪ ሂሳብ',

  'expires': 'ጊዜው የሚያበቃበት',
  'history': 'ታሪከ',
  'transfer': 'አስተላልፍ',
  'recent_transactions': 'የቅርብ ጊዜ የሂሳብ ዝውውሮች',

  'wallet_history': 'የሂሳብ ታሪከ',
  'wallet_transfer': 'ሂሳብ ያስተላልፉ',
  'transfer_to': 'ሂሳብ  የሚተላለፍለት ',
  'driver': 'አሽከርካሪ',

  'customer': 'ደንበኛ',
  'amount': 'መጠን',
  'reason': 'ምክንያት',
  'send': 'ላክ',

  'today': 'ዛሬ',
  'yesterday': 'ትላንት',

  // Promotions String
  'enter_promocode': 'የማስተዋወቂያ ኮዱን እዚህ ያስገቡ...',
  'apply': 'አመልክት',
  'current_points': 'አሁን ያሉ ነጥቦች',
  'expires_on': 'ጊዜው የሚያበቃበት',

  'exchange_points': 'ነጥብዎን ይቀይሩ',
  'my_coupons': 'የእኔ ኩፖኖች',
  'refer_friend': 'ለጓደኛን ያስተዋውቁ',
  'ride_rewards': 'የጉዞ ሽልማቶች',

  'delivery_rewards': 'ሽልማቶች',
  'convert_to_mobile_card': 'ወደ ሞባይል ካርድ ይቀይሩ',
  'transfer_points': 'ነጥቦችን ያስተላልፉ',
  'book_ride_with_points': 'በነጥብዎ ጉዞ ይጓዙ',
  'donate_points': 'ነጥቦን ይለግሱ',

  'transactions': 'ግብይቶች',
  'airtime_history': 'የአየር ሰዓት ታሪክ',
  'your_coupons': 'የእርስዎ ኩፖኖች',
  'points': 'ነጥቦች',
  'refer_friend_info':
      'ይህንን መተግበሪያ ለጓደኛዎ  እና ለቤተሰብዎ እንዲጠቀሙ በመጋበዝ ነጥቦችን ይሰብስቡ!, ያጋሩ',
  'your_referal_code': 'ያጋሩበት መለያ ቁጥር',
  'share': 'ያጋሩ',
  'share_referral': 'ውድ ተጠቃሚ መተግበሪያውን ከታክሲዬ ሪፈራል ለማውረድ የእኔን ኮድ @code ይጠቀሙ',

  'promo_details': 'የማስታወቂያ ዝርዝሮች',
  'detail': 'ዝርዝር',
  'promotion_code_success': 'የማስተዋወቂያ ኮድ በተሳካ ሁኔታ ተተግብሯል.',

  'buy_airtime': 'የአየር ሰዓት ይግዙ',
  'buy_airtime_info': 'መግዛት እንደሚፈልጉ እርግጠኛ ነዎት',
  'airtime': 'የአየር ሰዓት?',
  'buy': 'ይግዙ',
  'buy_airtime_success': 'የአየር ሰዓት በትክክል ተገዝቷዋል',

  'trasnsfer_success': 'በትክክል ተላልፏል',

  // Orders strings
  'your_orders': 'ትዕዛዝዎ',
  'completed': 'ተጠናቋል',
  'canceled': 'ተሰርዟል',
  'scheduled': 'በቀጠሮ የተያዘ ',

  'order_details': 'የትዕዛዝ ዝርዝሮች',
  'processed': 'ተሰርቷል',
  'cancelled': 'ተሰርዟል',
  'ride_scheduled': 'የቀጠሮ ጉዞ ተይዟል',

  'cancel_scheduled_ride': 'የቀጠሮ ጉዞ ይሰረዝ?',
  'cancel_scheduled_ride_info': 'እርግጠኛ ነዎት የቀጠሮ ጉዞውን መሰረዝ ይፈልጋሉ?',
  'cancel_schedule_success': 'የቀጠሮ ጉዞ ተሰርዟል!',

  'pick_up': 'መነሻ',
  'drop_off': 'መድረሻ',
  'time': 'ጊዜ',
  'total': 'ጠቅላላ',

  // Drivers strings
  'your_drivers': 'የእርሶ አሽከሪካሪዎች',
  'favourites': 'ምርጦች',
  'blocked': 'ታግዷል',

  'fav_driver_removed_success': 'ከምርጥ አሽከርካሪዎች ተወግዷል',
  'remove_fav_driver': 'የተመረጠው አሽከርካሪ ይወገድ?',
  'remove_fav_driver_info': 'እርግጠኛ ነዎት? ከምርጥ አሽከርካሪዎች ለማስወጣት ይፈልጋሉ?',

  // Payment strings
  'payment_methods': 'የመክፈያ ዘዴዎች',
  'add_new_method': 'አዲስ ዘዴ ጨምር',
  'default_method': 'ነባር ዘዴ',
  'cbe': 'የኢትዮጵያ ንግድ ባንክ',
  'mpessa': 'ኤም_ፔሳ',
  'telebirr': 'ቴሌ ብር',
  'select_partner': 'አጋርን ምረጥ',
  'payment': 'ክፍያ',
  'hello_cash_pay_fare': 'እባክዎ *912# ይደውሉ እና ክፍያውን ያጠናቅቁ',

  'new_payment_method': 'አዲስ የመክፈያ ዘዴ ምረጥ',
  'edit': 'አስተካክል',
  'remove': 'አስወግድ',
  'setDefault': 'እንደ ነባር ዘዴ መዝግብ',

  'hello_cash': 'ሄሎ ካሽ',

  // app settings strings
  'sound_voice': 'ድምጽ',
  'customize_sounds': 'የመተግበሪያ ድምጾችን እና ሌሎችንም አስተካክል',
  'navigation': 'አቅጣጫ',
  'google_maps': 'የጎግል ካርታዎች',

  'night_mode': 'የምሽት እይታ',
  'night_mode_info': 'የምሽት እይታ አብራ ወይም አጥፋ',
  'follow_my_ride': 'ጉዞዬን ተከተል',
  'follow_my_ride_info': 'ጉዞዎችን በቀላሉ ለቤተሰብ እና ለጓደኞች አጋራ',

  'language': 'ቋንቋ',
  'english': 'እንግሊዝኛ',
  'customize_privacy': 'ከእኛ ጋር የሚያጋሩትን ያስተካክሉ',

  'privacy_settings_detail':
      'ነፃነትዎን አንጋፋም...በየጊዜው የሚደርስዎትን ማሳወቂያዎች በመመልከት ለእርስዎ የሚያስፈልግዎትን ብቻ መጥነን እንድንልክ ልምዶትን ያጋሩን ',

  // Legals strings
  'view_terms_conditions': 'የመተግበሪያ አጠቃቀም ደንብና መመሪያዎች ይመልከቱ',
  'privacy_policy': 'ግላዊ መመሪያ',
  'privacy_info': 'የእርስዎን መረጃ እንዴት እንደምንጠቀም ይመልከቱ',
  'about_us': 'ስለ እኛ',
  'know_us': 'ስለ እኛ ትንሽ ይወቁ',

  // common texts
  'add_phone_number': 'እባክዎ ስልክ ቁጥርዎን ያስገቡ',
  'done': 'ተከናውኗል',
  'yes': 'አዎ',
  'no': 'አይ',

  //notification Strings
  'ride_completed': 'ጉዞ ተጠናቋል',
  'your_fare_was': 'የጉዞዎ ዋጋ',

  // Empty state strings
  'empty_state_title': 'ይቅርታ! ምንም መረጃ አልተገኘም',

  // Error messages
  'invalid_passcode': 'ልክ ያልሆነ የይለፍ ቃል',
  'password_match_error': 'የይለፍ ቃልዎ አይዛመድም',
  'error_name': 'እባክዎ ሙሉ ስም ያስገቡ',
  'error_email': 'ልክ ያልሆነ ኢሜይል አድራሻ',

  'error_phone': 'እባክዎ ስልክ ቁጥር ያስገቡ',
  'invalid_phone': 'እባክዎ የሚሰራ ስልክ ቁጥር ያስገቡ',
  'error_amount': 'ልክ ያልሆነ መጠን',

  'api_error': 'ልክ ያልሆነ Api',
  'pickup_location_error': 'እባክዎ መነሻ ቦታዎን ይምረጡ',
  'destination_error': 'እባክዎ መድረሻዎን ይምረጡ',
  'vehicle_select_error': 'እባክዎ ተሽከርካሪ ይምረጡ',

  'error_label': 'እባክዎ የአድራሻ መለያ ያስገቡ',
  'place_not_picked_error': 'እባክዎ ቦታ ይምረጡ',
  'check_pending_operations': 'እባክዎ በመጠባበቅ ላይ ያሉ ስራዎችን መጀመሪያ ያጠናቅቁ',

  'invalid_time': 'ልክ ያልሆነ ጊዜ!\ሰዓቱ ከ30 ደቂቃዎች በኋላ መሆን አለበት',
  'error_connection': 'መገናኘት አልተቻለም፣ እባክዎ ኢንተርኔትዎን ያስተካክሉ እና እንደገና ይሞክሩ',
  'fetch_ride_summary_error': 'የጉዞውን ማጠቃለያ ማምጣት አልተቻለም',
  'network_error': 'ኦፕሬሽን ማከናወን አልተቻለም...እባክዎ የኢንተርኔት ግንኙነትዎን ያረጋግጡ!',

  'error_select_reason': 'እባክዎ ምክንያትዎን ይምረጡ',
  'error_promotion_code': 'እባክዎ የማስተዋወቂያ ኮድ ያቅርቡ',
  'error_delivery_detail': 'እባክዎ የመልእክቱን ዝርዝር መረጃ ያስገቡ',

  // New Translations I - the following texts are not translated to other languages
  'logout': 'ውጣ',
  'favourite': 'የሚወደድ',
};
